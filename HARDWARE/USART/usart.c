...
void USART3_IRQHandler(void)                	
{
      ...
      Time_UART3=0;
	    Res3= USART_ReceiveData(USART3);
			Uart3_Buf[First_Int3++] = Res3;		//将接收到的字符串存到缓存中
			if(First_Int3++ > 128)       		//如果缓存满,将缓存指针指向缓存的首地址
			{
				First_Int3 = 0;
			}
...
void UART3_Data(char byte)
{
		...
		USART_SendData(USART3 ,byte);//发送当前字符
}
/*******************************************************************************
